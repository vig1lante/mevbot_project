...
class CONSTANTS:
    ...
    def get_str_from_config(self, field, config):
        ...
        config_value = config[field]
        if config_value is not None and config_value != '' and isinstance(config_value, str):
            return config_value
        else:
...
class SERVICE_SETTINGS(CONSTANTS):
    ...
    def __init__(self, config):
        if config is not None:
            self.BNB_USDT_CONTRACT_ADDRESS = self.get_str_from_config("BNB_USDT_CONTRACT_ADDRESS", config)
